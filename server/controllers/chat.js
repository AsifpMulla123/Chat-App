    ...
    const transformedChats = chats.map((_id, name, members, groupChat) => {
        ...
        return {
            ...
            avatar: groupChat ? members.slice(0, 3).map(({ avatar }) =>
                avatar.url) : [otherMember.avatar.url],
...
const sendAttachments = TryCatch(async (req, res, next) => {
    ...
    // uploading files to the cloudnary
    const attachments = [];
...
const deleteChat = TryCatch(async (req, res, next) => {
    ...
    emitEvent(erq, REFETCH_CHATS, members);
...
export { newGroupChat, getMyChats, getMyGroups, addMembers, removeMembers, leaveGroup, sendAttachments, getChatDetails, renameGroup, deleteChat };